...
## 项目结构
与构建系统直接相关的核心位置如下：
- app/Codexling/Package.swift：SPM 包定义，描述目标、依赖、资源与平台等。
- app/Codexling/rebuild_and_run.sh：用于快速重建并运行应用的脚本。
- app/Codexling/package_app.sh：用于打包应用（生成 .app 或安装包）。
- app/Codexling/release_app.sh：用于发布流程（签名、归档、上传等）。
- app/Codexling/scripts/run_chatgpt_api_probe.sh：辅助脚本，用于探测外部 API 连通性。

```mermaid
graph TB
A["app/Codexling/Package.swift"] --> B["rebuild_and_run.sh"]
A --> C["package_app.sh"]
A --> D["release_app.sh"]
