...
## 简介
本指南面向 Codexling 项目的开发与维护者，提供覆盖单元测试、集成测试与 UI 测试的完整策略与实现方法。文档基于仓库中现有的 Swift Package 测试结构与 Next.js 前端工程配置，给出：
- 测试框架使用方式与用例编写规范
- 模拟对象、测试数据与测试环境配置
- 覆盖率要求与持续集成中的自动化测试流程
- 常见测试场景示例与调试技巧
...
## 项目结构
本项目包含两个主要子系统：
- macOS 应用（Swift Package）：位于 app/Codexling，包含应用源码与 Xcode/SwiftPM 测试目标
- 着陆页网站（Next.js）：位于 app/landing，包含前端页面与组件

```mermaid
graph TB
subgraph "macOS 应用"
A["app/Codexling/Sources"] --> B["app/Codexling/Tests"]
A --> C["app/Codexling/Package.swift"]
end
subgraph "Next.js 站点"
...
- Swift 测试目标
  ...
  - 测试入口与样例位于 Tests/CodexlingTests/CodexlingTests.swift
- Next.js 前端工程
...
- 测试目标与入口
  - 测试目标由 Package.swift 声明，测试类与用例位于 Tests/CodexlingTests/CodexlingTests.swift
- 用例编写规范
