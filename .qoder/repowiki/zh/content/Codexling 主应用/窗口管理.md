...
## 简介
本文件面向 Codexling 的窗口子系统，重点围绕 DetachedWindowController 的窗口创建、生命周期与状态保持机制展开，并系统阐述多窗口支持、窗口间通信与数据同步策略。同时覆盖窗口布局管理、最小化/最大化行为、全屏模式处理、权限管理与沙箱安全、系统集成能力，以及性能优化、内存泄漏检测与调试技巧。文档力求在技术深度与可读性之间取得平衡，帮助开发者快速上手并高效扩展窗口功能。

## 项目结构
Codexling 的 macOS 应用采用 Swift 开发，窗口相关逻辑集中在 app/Codexling/Sources/Codexling 目录下：
