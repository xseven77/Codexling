...
/// Writes the Codexling Gateway into DeepSeek Harness as a `llm-pi-ai` route.
///
...
///
/// - `~/.dsh/settings.yaml` → `llm-pi-ai.providers.codexling`
...
/// scoped** — only the Codexling route span and the one credential line are
...
struct DSHGatewayConfigurator: Sendable {
    ...
    static let providerDisplayName = "Codexling Gateway"
    ...
    /// Codexling route span is rewritten from scratch, which drops removed
    ...
    /// Insert or replace the Codexling route inside the `llm-pi-ai` block,
    ...
    /// Remove the Codexling route, then collapse a `providers` mapping and a
...
enum NarrowYAML {
    ...
    /// Locate the Codexling route whether or not the `llm-pi-ai` block's own
