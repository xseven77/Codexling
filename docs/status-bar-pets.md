...
Codexling 在 macOS 状态栏用前置圆灯表达 Codex 任务状态，用文字展示有效额度窗口；胶囊始终使用固定透明中性色，用户选择的提醒色只改变文字。当前 Pet 的动画统一显示在主窗口和悬停卡片中。
...
- 前置圆灯仍严格表达任务状态，不随额度色改变；额度色和任务色是两套独立语义。
- 状态栏使用单层 AppKit 自绘，背景、文字、圆灯、活动 Wave 和点击波纹在同一坐标系内完成，避免系统材质裁剪自定义内容。
...
- 禁用 `NSStatusBarButton` 默认的大胶囊按压高亮；点击时由自绘视图从实际点击位置播放一次 Material ripple。执行任务时的持续 Wave 独立从左侧任务圆灯向四周扩散，两者都裁剪在胶囊轮廓内。
...
- “胶囊提醒色”：`neutral`、`automatic`、绿、黄、红、灰；只改变文字。
- “活动状态 Wave”：非空闲状态下控制从圆灯向外扩散的状态栏 Wave。
