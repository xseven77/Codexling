# Codexling 状态栏与 Pets 方案与实现

> **时代标注**：本文写于 0.3.x 单 Codex 时代，详尽记录了当时状态栏胶囊与 Pet 体系
> 的方案与实现，保留作演进档案。0.4+ 起状态栏已演进为多 Agent / 多供应商形态
> （agent 状态 + 供应商额度双维度、账号轮播、活动波浪、刘海面板），Pet 体系扩展为
> 10 只内置宠物与独立宠物窗。**现行行为**以操作手册
> [07-菜单栏与窗口](manual/07-菜单栏与窗口.md) 与 [04-桌面宠物](manual/04-桌面宠物.md)
> 为准；本文中与手册冲突的细节以手册和源码为准。

## 1. 目标

Codexling 在 macOS 状态栏默认用前置圆灯表达 Codex 任务状态，也可切换为额度健康色；文字展示有效额度窗口，胶囊始终使用固定透明中性色。当前 Pet 的动画统一显示在主窗口和悬停卡片中。
...
- 胶囊提醒色提供 `automatic`、`neutral`、`green`、`yellow`、`red`、`gray` 枚举。自动模式按主额度健康度映射绿、黄、红、灰；设置页也可以选择固定颜色。
- 提醒色默认值为 `neutral`；用户可选择 `automatic` 跟随额度。旧版任务状态固定色值（如 `blue`、`purple`、`cyan`、`amber`）不属于新枚举，读取时会回退为 `neutral`。
- 提醒色沿用主窗口右上角 `QuotaAtAGlanceChip` 的绿、黄、红、灰语义，但只作用于文字。背景固定为透明中性色，不使用 `NSGlassEffectView`，不会根据菜单栏壁纸亮度自动翻成亮底或暗底。
- 前置圆灯默认表达任务状态；设置页可切换为额度健康色，额度色和任务色仍是两套独立语义。
- 状态栏使用单层 AppKit 自绘，背景、文字、圆灯、活动状态流光和点击波纹在同一坐标系内完成，避免系统材质裁剪自定义内容。
...
- 禁用 `NSStatusBarButton` 默认的大胶囊按压高亮；点击时由自绘视图从实际点击位置播放一次 Material ripple。执行任务时显示顺时针旋转的边缘流光，两种效果都裁剪在胶囊轮廓内。
...
![额度语义色与任务圆灯矩阵](../assets/screenshots/codexling-status-capsule-matrix.png)
...
有主、次两个有效额度窗口时会同时显示，例如 `● 工作中·5h 42%·周 77%`。状态栏不显示 Pet 图片；圆灯默认跟随任务状态，也可在设置页切换为额度健康色。
...
- “状态圆灯颜色”：选择跟随任务状态、额度状态或固定单色。
- “活动流光”：任务活动时控制状态栏与 Pet 状态胶囊的顺时针边缘流光。
- “活动流光颜色”：选择跟随任务状态或固定单色。
...
codexling.statusBarIndicatorColorMode
